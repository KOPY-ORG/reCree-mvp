...
export function PlaceMarker({ color, isSelected, isSaved, nameEn, postCount }: PlaceMarkerProps) {
  ...
  return (
    ...
    >
      ...
      {/* 숫자 배지 — 동그라미 글리프 중앙에 겹침, saved일 때 미표시 */}
      {!isSaved && postCount >= 2 && (
        <div
          className="absolute pointer-events-none"
          style={{ top: GY, left: GX, transform: "translate(-50%, -50%)" }}
        >
          <span
            className="font-bold leading-none text-zinc-900"
            style={{ fontSize: countLabel.length > 1 ? 6.5 : 8 }}
          >
            {countLabel}
          </span>
        </div>
