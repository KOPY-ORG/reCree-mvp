...
export function TopicDialog({ open, topic, defaultParentId, allTopics, parentEffectiveColor, onClose, onSaved }: TopicDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      ...
      if (isEdit && topic) {
        ...
        setColorHex(topic.colorHex ?? parentEffectiveColor?.hex ?? "#D3FD52");
        ...
        setColorHex(parentEffectiveColor?.hex ?? "#D3FD52");
  ...
  const previewHex = inheritColor ? (parentEffectiveColor?.hex ?? "#D3FD52") : colorHex;
